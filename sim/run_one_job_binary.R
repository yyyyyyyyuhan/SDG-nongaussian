# binary outcome
# usage: Rscript simulation/run_one_job_binary.R <structure> <lam_g> <lam_t>
...
source("R/binary/mcem_cgd.R")
source("R/binary/generate_data.R")
...
if (is.null(data$Y_list) || is.null(data$True_Gamma) || is.null(data$True_Theta)) {
  stop("generate_data() must return a list with Y_list.")
}
...
fit <- tryCatch(
  mcem_cgd_binary(Y_list = Y_list, lambda_gamma = lam_g, lambda_theta = lam_t,
                  max_iter = 100, n_samples = 100, burn_in = 10,
